...
def parse_duration(duration_str):
    ...
    total_duration = timedelta(days=time_params['D'], hours=time_params['H'], minutes=time_params['M'], seconds=time_params['S'])
    return total_duration   

def transform_data(row):
    total_duration= parse_duration(row['Duration'])
    row['Duration']=(datetime.min + total_duration).time()
